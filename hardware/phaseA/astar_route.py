...
CLR = 0.155
VIA_R = 0.3
X0, Y0, X1, Y1 = 10.0, 0.8, 157.5, 49.2
NX = int((X1 - X0) / GRID) + 1
NY = int((Y1 - Y0) / GRID) + 1
F, Bc = 0, 1
...
# (net, depart, arrivee, echappement_depart, echappement_arrivee)
jobs = [
    ("A9",    (24.16, 44.0, Bc),   (124.14, 35.38, F), None, (-1, 0)),
    ("A12",   (29.24, 46.54, Bc),  (124.14, 37.33, F), None, (-1, 0)),
    ("D3",    (36.86, 44.0, Bc),   (124.14, 18.68, F), None, (-1, 0)),
    ("IRQ_n", (49.56, 46.54, Bc),  (124.14, 10.32, F), None, (-1, 0)),
    ("D5V2",  (129.86, 18.02, F),  (156.95, 14.84, Bc), (1, 0), (-1, 0)),
]

def clear_escape(grids, layer, x, y, direction, own, length=1.6):
    """Rend au net `own` les cellules du couloir d'échappement d'une pastille."""
    gx, gy = cell(x, y)
    ...
    if direction is None:
        ...
        for k in range(steps + 1):
            rng += [(gx + dx * k + (dy != 0) * o, gy + dy * k + (dx != 0) * o)
                    for o in (-0, )]  # couloir de 1 cellule de large
        rng += [(gx + ox, gy + oy) for ox in (-1, 0, 1) for oy in (-1, 0, 1)]
    for x2, y2 in rng:
        if 0 <= x2 < NX and 0 <= y2 < NY:
...
grids = raster()
for net, (sx, sy, sl), (tx, ty, tl), esc_s, esc_t in jobs:
    nc = netcode(net)
    clear_escape(grids, sl, sx, sy, esc_s, nc + 1)
    clear_escape(grids, tl, tx, ty, esc_t, nc + 1)
    path = astar(grids, nc, sx, sy, sl, tx, ty, tl)
    if path is None:
        print("ECHEC A*", net)
        continue
    emit(path, nc, sx, sy, tx, ty)
    # marquer le chemin comme obstacle du net pour les suivants
    for l, gx, gy in path:
        for oy in (-1, 0, 1):
            for ox in (-1, 0, 1):
                x2, y2 = gx + ox, gy + oy
