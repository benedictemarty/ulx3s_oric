...
def via_ok(board, pos, items, gnd):
    ...
    vshape = pcbnew.SHAPE_CIRCLE(pos, VIA_DIA // 2)
    hshape = pcbnew.SHAPE_CIRCLE(pos, VIA_DRILL // 2)
    for it in items:
        try:
            s = it.GetEffectiveShape()
        except Exception:
            continue
        if s.Collide(vshape, int(CLEARANCE)):
            return False
        ...
        if isinstance(it, pcbnew.PAD) and it.GetDrillSize().x > 0:
            ...
            if dh.Collide(hshape, int(HOLE_CLEAR)):
                return False
    ...
    for it_pass in range(8):
        ...
        for fp in board.GetFootprints():
            for p in fp.Pads():
                if p.GetNetCode() != gnd:
                    continue
                if conn.GetConnectedItemsCount(p) <= 1:
                    todo.append(p)
        added = 0
        for p in todo:
            base = p.GetPosition()
